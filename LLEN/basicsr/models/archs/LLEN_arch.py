...
class Enhancer(nn.Module):
    def __init__(self, n_fea_middle, n_fea_in=4, n_fea_out=3):
        ...
        self.additional_depth_conv = nn.Conv2d(n_fea_middle, n_fea_middle, kernel_size=5, padding=2, bias=True, groups=n_fea_middle)
        self.conv2 = nn.Conv2d(n_fea_middle, n_fea_out, kernel_size=1, bias=True)
        # 添加处理拼接输出的卷积层
        self.concat_conv = nn.Conv2d(n_fea_middle + n_fea_out, n_fea_middle, kernel_size=1, bias=True)

    def forward(self, img):
        mean_c = img.mean(dim=1).unsqueeze(1)
        input = torch.cat([img, mean_c], dim=1)
        x_1 = self.conv1(input)
        illu_fea = self.depth_conv1(x_1)
        illu_fea = self.additional_depth_conv(illu_fea)
        illu_map = self.conv2(illu_fea)
        concat_output = torch.cat([illu_fea, illu_map], dim=1)
...
class Denoiser(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        ...
        # print("Denoiser：level:", level)
        # print("Denoiser：num_blocks:", num_blocks)
        self.dim = dim
        self.level = level
...
class LLEN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, level=2, num_blocks=[1,1,1]):
        super(LLEN, self).__init__()
        self.stage = stage
        modules_body = [LLEN_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=level, num_blocks=num_blocks)
...
#     model = LLEN(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
#     print(model)
